...
public class ShulkerCharmItem extends Item implements ITrinket, CustomDurabilityItem {
		...
		if (ShulkerCharm.CHARM_FLIGHT.grants(player, VanillaAbilities.ALLOW_FLYING)) {
			if (power == 0) {
				...
				if (!VanillaAbilities.ALLOW_FLYING.isEnabledFor(player)) {
					...
					player.addPotionEffect(new StatusEffectInstance(StatusEffects.SLOW_FALLING, 200));
				}
			}
		} else {
			if (power > 0) {
	...
	public void onUnequip(PlayerEntity player, ItemStack stack) {
		if (ShulkerCharm.CHARM_FLIGHT.grants(player, VanillaAbilities.ALLOW_FLYING)) {
			...
			if (!VanillaAbilities.ALLOW_FLYING.isEnabledFor(player)) {
				player.abilities.flying = false;
				player.addPotionEffect(new StatusEffectInstance(StatusEffects.SLOW_FALLING, 100));
			}
		}
	}
	...
	public boolean shouldShowDurability(ItemStack stack) {
		...
		if (tag.containsKey("Power", NbtType.INT)) return tag.getInt("Power") != getMaxDurability(stack);
		else return true;
	}
	...
	public int getPower(ItemStack stack) {
		...
		if (!tag.containsKey("Power", NbtType.INT)) tag.putInt("Power", 0);
